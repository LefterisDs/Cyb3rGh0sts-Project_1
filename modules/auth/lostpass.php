...
if (isset($_REQUEST['do']) && $_REQUEST['do'] == "go") {
	...
	$hash = $_REQUEST['h'];
	$res = db_query("SELECT `user_id`, `hash`, `password`, `datetime` 
                      FROM   passwd_reset
			          WHERE    `user_id` = '" . mysql_escape_string($userUID) . "'
			               AND `hash` = '" . mysql_escape_string($hash) . "'
			               AND TIME_TO_SEC(TIMEDIFF(`datetime`,NOW())) < 3600", $mysqlMainDb);
...
} elseif ((!isset($email) || !email_seems_valid($email) || !isset($userName) || empty($userName)) && !isset($_REQUEST['do'])) {
    if (preg_match("/[^a-zA-Z0-9]/", $userName) ){
        header("Location: " . $_SERVER['PHP_SELF']);
        exit();
    }
    ...
    if (isset($email) and !email_seems_valid($email)) {
        ...
        $tool_content .= '<table width="99%">
                            <tbody>
                                <tr>
                                    <td class="caution">' .
                                        '<p><strong>' . $lang_pass_invalid_mail . '<br />&nbsp;<br />' . '&nbsp;<br />&nbsp;<br /></strong></p>' .
                                    '</td>
                                </tr>
                            </tbody>
                          </table>';
